...
import type { ApprovalQueueRecord, ApprovalQueueStatus, ApprovalValidationStatus } from "../_types"
...
function transferStatusLabel(status: ApprovalQueueStatus): string {
...
function transferStatusClass(status: ApprovalQueueStatus): string {
...
function validationLabel(status: ApprovalValidationStatus): string {
  if (status === "ok") return "OK"
  if (status === "hata") return "RED"
  return "-"
}

function validationClass(status: ApprovalValidationStatus): string {
  if (status === "ok") return "text-emerald-700"
  if (status === "hata") return "text-red-600"
  return "text-slate-400"
}

export function getApprovalQueueColumns(
  onViewDetail: (row: ApprovalQueueRecord) => void,
  onManualApprove: (row: ApprovalQueueRecord) => void,
): ColumnDef<ApprovalQueueRecord>[] {
  return [
    {
