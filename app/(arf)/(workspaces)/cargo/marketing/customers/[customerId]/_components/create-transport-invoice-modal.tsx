...
type TransportMode = {
  mode: "transport"
  selectedTransports: CustomerTransportRecord[]
  selectedGelirler?: never
  tasimaNo?: never
}

type GelirMode = {
  mode: "gelir"
  selectedGelirler: GelirKalemi[]
  tasimaNo: string
  selectedTransports?: never
}
...
} & (TransportMode | GelirMode)

/* ─── Component ─── */

export function CreateTransportInvoiceModal(props: ModalProps) {
  const { open, onOpenChange, customerInfo, onConfirm, mode } = props
  ...
  const items = isTransport ? props.selectedTransports : props.selectedGelirler
  const itemCount = items.length

  const vatRate = 0.20
  const tevkifatRate = tevkifat === "yok" ? 0 : 2 / 10

  // Gelir modunda seçili kalemlerden gelen tevkifat değeri (varsa)
  const derivedGelirTevkifat = useMemo(() => {
    if (isTransport) return null
    return props.selectedGelirler.find((g) => g.tevkifat && g.tevkifat !== "yok")?.tevkifat ?? null
  }, [isTransport, props.selectedGelirler])

  const subTotal = useMemo(() => {
    if (isTransport) return props.selectedTransports.reduce((acc, t) => acc + t.satisFiyat, 0)
    return props.selectedGelirler.reduce((acc, g) => acc + g.birimFiyat, 0)
  }, [isTransport, items])

  const vatTotal = useMemo(() => {
    if (isTransport) return subTotal * vatRate
    return props.selectedGelirler.reduce((acc, g) => acc + g.kdvTutar, 0)
  }, [isTransport, items, subTotal])

  const tevkifatTotal = useMemo(() => {
    if (isTransport) return vatTotal * tevkifatRate
    return props.selectedGelirler.reduce((acc, g) => acc + g.tevfikatTutar, 0)
  }, [isTransport, items, vatTotal, tevkifatRate])

  const grandTotal = useMemo(() => {
    if (isTransport) return subTotal + vatTotal - tevkifatTotal
    return props.selectedGelirler.reduce((acc, g) => acc + g.toplamTutar, 0)
  }, [isTransport, items, subTotal, vatTotal, tevkifatTotal])
  ...
  useEffect(() => {
    ...
    if (isTransport) {
      setInvoiceName(`${customerInfo.tradeName} - ${itemCount} Taşıma Faturası`)
    } else {
      setInvoiceName(`${props.tasimaNo} - ${customerInfo.tradeName} Satış Faturası`)
    }
  }, [open, customerInfo.tradeName, itemCount, isTransport, baseData])
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="max-h-[90vh] max-w-5xl! w-[90vw] overflow-y-auto">
        <AlertDialogHeader>
          <AlertDialogTitle>Taşıma Faturası Oluştur</AlertDialogTitle>
        </AlertDialogHeader>
        ...
        <div className="space-y-4">
                ...
                <div className="border-t border-slate-200 pt-3 space-y-3">
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <Checkbox id="note-bakiye-transport" checked={notaBakiyeEkle} onCheckedChange={(checked: boolean | "indeterminate") => setNotaBakiyeEkle(checked === true)} />
                      <Label htmlFor="note-bakiye-transport" className="text-sm text-slate-700">Müşteri bakiyesini not olarak ekle</Label>
                    </div>
                  </div>
          ...
          {/* ── Hizmet / Ürün Satırları (Seçili Kalemler) ── */}
          <Card className="rounded-2xl border-slate-200 shadow-sm">
            <CardHeader className="pb-2">
            ...
            <CardContent className="space-y-3">
              <div className="max-h-72 space-y-3 overflow-y-auto">
                {isTransport
                  ? props.selectedTransports.map((transport, index) => {
                      const lineVat = transport.satisFiyat * vatRate
                      const lineTotal = transport.satisFiyat + lineVat

                      return (
                        <div key={transport.id} className="rounded-xl border border-slate-200 bg-white p-3">
                          <div className="mb-2 text-xs font-medium text-slate-400">Seçili Taşıma {index + 1}</div>
                          <div className="grid gap-3 lg:grid-cols-12">
                            <div className="lg:col-span-3">
                              <Label className="mb-1.5 text-xs text-slate-600">Hizmet / Ürün</Label>
                              <Input readOnly value={transport.tasimaNo} className="h-11 bg-slate-50 cursor-default border-slate-200 font-medium text-slate-700 focus-visible:ring-0 focus-visible:ring-offset-0 select-none" />
                            </div>

                            <div className="lg:col-span-1">
                              <Label className="mb-1.5 text-xs text-slate-600">Miktar</Label>
                              <Input readOnly value={String(transport.toplamAdet || 1)} className="h-11 bg-slate-50 cursor-default border-slate-200 text-slate-600 focus-visible:ring-0 focus-visible:ring-offset-0 select-none" />
                            </div>

                            <div className="lg:col-span-2">
                              <Label className="mb-1.5 text-xs text-slate-600">Birim</Label>
                              <Input readOnly value={transport.yukTipleri || "Adet"} className="h-11 bg-slate-50 cursor-default border-slate-200 text-slate-600 focus-visible:ring-0 focus-visible:ring-offset-0 select-none" />
                            </div>

                            <div className="lg:col-span-2">
                              <Label className="mb-1.5 text-xs text-slate-600">Fiyat</Label>
                              <Input readOnly value={formatNumber(transport.satisFiyat)} className="h-11 bg-slate-50 cursor-default border-slate-200 text-slate-600 focus-visible:ring-0 focus-visible:ring-offset-0 select-none" />
                            </div>

                            <div className="lg:col-span-2">
                              <Label className="mb-1.5 text-xs text-slate-600">Vergi</Label>
                              <Input readOnly value="%20" className="h-11 bg-slate-50 cursor-default border-slate-200 text-slate-600 focus-visible:ring-0 focus-visible:ring-offset-0 select-none" />
                            </div>

                            <div className="lg:col-span-2">
                              <Label className="mb-1.5 text-xs text-slate-600">Toplam</Label>
                              <Input readOnly value={formatNumber(lineTotal)} className="h-11 bg-slate-50 cursor-default border-slate-200 text-slate-600 focus-visible:ring-0 focus-visible:ring-offset-0 select-none" />
                            </div>
                          </div>
                        </div>
                      )
                    })
                  : props.selectedGelirler.map((gelir, index) => (
                      <div key={gelir.id} className="rounded-xl border border-slate-200 bg-white p-3">
                        <div className="mb-2 text-xs font-medium text-slate-400">Seçili Gelir Kalemi {index + 1}</div>
                        <div className="grid gap-3 lg:grid-cols-12">
                          <div className="lg:col-span-3">
                            <Label className="mb-1.5 text-xs text-slate-600">Hizmet / Ürün</Label>
                            <Input readOnly value={gelir.aciklama} className="h-11 bg-slate-50 cursor-default border-slate-200 font-medium text-slate-700 focus-visible:ring-0 focus-visible:ring-offset-0 select-none" />
                          </div>

                          <div className="lg:col-span-1">
                            <Label className="mb-1.5 text-xs text-slate-600">Miktar</Label>
                            <Input readOnly value="1" className="h-11 bg-slate-50 cursor-default border-slate-200 text-slate-600 focus-visible:ring-0 focus-visible:ring-offset-0 select-none" />
                          </div>

                          <div className="lg:col-span-2">
                            <Label className="mb-1.5 text-xs text-slate-600">Birim</Label>
                            <Input readOnly value="Hizmet" className="h-11 bg-slate-50 cursor-default border-slate-200 text-slate-600 focus-visible:ring-0 focus-visible:ring-offset-0 select-none" />
                          </div>

                          <div className="lg:col-span-2">
                            <Label className="mb-1.5 text-xs text-slate-600">Fiyat</Label>
                            <Input readOnly value={formatNumber(gelir.birimFiyat)} className="h-11 bg-slate-50 cursor-default border-slate-200 text-slate-600 focus-visible:ring-0 focus-visible:ring-offset-0 select-none" />
                          </div>

                          <div className="lg:col-span-2">
                            <Label className="mb-1.5 text-xs text-slate-600">Vergi</Label>
                            <Input readOnly value={formatTaxRatePercent(gelir.kdvOran)} className="h-11 bg-slate-50 cursor-default border-slate-200 text-slate-600 focus-visible:ring-0 focus-visible:ring-offset-0 select-none" />
                          </div>

                          <div className="lg:col-span-2">
                            <Label className="mb-1.5 text-xs text-slate-600">Toplam</Label>
                            <Input readOnly value={formatNumber(gelir.toplamTutar)} className="h-11 bg-slate-50 cursor-default border-slate-200 text-slate-600 focus-visible:ring-0 focus-visible:ring-offset-0 select-none" />
                          </div>
                        </div>
                        {gelir.tevkifat && gelir.tevkifat !== "yok" && (
                          <div className="mt-3 border-t border-slate-100 pt-3">
                            <Label className="mb-1.5 text-xs text-slate-600">Tevkifat Oranı</Label>
                            <div className="flex items-center gap-2">
                              <Select value={gelir.tevkifat} disabled>
                                <SelectTrigger className="h-9 w-52 cursor-default text-sm opacity-80">
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                  <SelectItem value={gelir.tevkifat}>{gelir.tevkifat} Tevkifat Uygula</SelectItem>
                                </SelectContent>
                              </Select>
                            </div>
                          </div>
                ...
                {itemCount === 0 && (
                  <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-500">
                    Faturalandırılacak satır bulunamadı.
                  </div>
                )}
              </div>
            </CardContent>
          ...
          <Card className="rounded-2xl border-slate-200 shadow-sm">
            ...
            <CardContent className="space-y-0">
              ...
              <div className="flex items-center justify-between border-t border-slate-200 py-3 text-sm text-slate-700">
                ...
                <span className="tabular-nums font-medium">{formatCurrency(vatTotal)}</span>
              </div>
              <div className="flex items-center justify-between border-t border-slate-200 py-3 text-sm text-slate-700">
                <div className="flex items-center gap-2">
                  <span>Tevkifat</span>
                  ...
                  ) : derivedGelirTevkifat ? (
                    <Select value={derivedGelirTevkifat} disabled>
                      <SelectTrigger className="h-7 w-28 text-xs cursor-default opacity-80">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value={derivedGelirTevkifat}>{derivedGelirTevkifat}</SelectItem>
                      </SelectContent>
                    </Select>
                  ) : null}
                </div>
                <span className="tabular-nums font-medium">
                  {isTransport
                    ? tevkifat === "yok" ? "-" : formatCurrency(tevkifatTotal)
                    : tevkifatTotal > 0 ? formatCurrency(tevkifatTotal) : "-"}
                </span>
              </div>
              <div className="flex items-center justify-between border-t border-slate-200 pt-3 text-sm text-slate-700">
                <span>Genel Toplam</span>
                <span className="text-base tabular-nums font-semibold text-slate-900">{formatCurrency(grandTotal)}</span>
              </div>
            </CardContent>
          </Card>
        </div>
