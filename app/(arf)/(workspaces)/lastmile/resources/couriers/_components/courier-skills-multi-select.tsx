...
import { resolveCourierSkillLabel } from '../_lib/query-couriers'
import type { CourierSkill, CourierSkillOption } from '../_types/courier'

type Props = {
  value: CourierSkill[]
  onChange: (next: CourierSkill[]) => void
...
export function CourierSkillsMultiSelect({
  value,
  onChange,
  options,
  isLoading = false,
  ...
  const toggle = (skill: CourierSkill) => {
  ...
  const selectedLabels = value.map((skill) => resolveCourierSkillLabel(skill, labelMap))
      ...
      >
        <div className='max-h-56 space-y-0.5 overflow-y-auto'>
          {options.length === 0 ? (
            ...
            options.map((option) => {
              const code = option.code as CourierSkill
