...
import { getCourierCostList, updateCourierCostList } from '../../_api/courier-cost-api'
...
export default function CourierCostListDetailPageContent() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [detail, zoneRows] = await Promise.all([getCourierCostList(id), listPriceZones()])
      if (!detail) {
          ...
          <CardContent className='p-4 lg:p-5'>
            <Tabs value={tab} onValueChange={setTab}>
              ...
              <TabsContent value='couriers' className='mt-4'>
                <CourierCostCouriersSimPanel costListId={list.id} />
              </TabsContent>
            </Tabs>
          </CardContent>
        </Card>
