...
type Props = {
  ...
  onToggleStatus: (id: string, status: PriceList['status']) => void
}
  ...
  if (lists.length === 0) {
    return (
      <div className='rounded-2xl border border-dashed border-slate-200 bg-slate-50/50 px-6 py-16 text-center'>
        <p className='text-sm font-medium text-slate-700'>Henüz fiyat listesi yok</p>
        <p className='mt-1 text-sm text-slate-500'>
          Varsayılan veya müşteriye özel bir ücretlendirme oluşturun.
        </p>
      </div>
    )
  }
            ...
            <tr key={list.id} className='hover:bg-slate-50/60'>
              ...
              <td className='px-4 py-3'>
                <div className='flex items-center gap-2'>
                  <Link
                    href={ARF_ROUTES.lastmile.finance.priceLists.detail(list.id)}
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align='end'>
                    <DropdownMenuItem asChild>
                      <Link href={ARF_ROUTES.lastmile.finance.priceLists.detail(list.id)}>
                        Detay
                      </Link>
                    </DropdownMenuItem>
