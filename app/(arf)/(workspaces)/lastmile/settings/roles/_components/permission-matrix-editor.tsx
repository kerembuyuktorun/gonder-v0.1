...
import type { ModuleCategory, PermissionDefinition, RolePermissions } from '../_types/role'
...
export function PermissionMatrixEditor({
  ...
  const grouped = useMemo(() => {
    ...
    categories.forEach((category) => {
      const rows = definitions.filter((item) => item.moduleCategoryId === category.id)
  ...
  const columns = useMemo<ColumnDef<MatrixRow>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const standard = row.original.permissions.filter(
            (item) => item.permissionType !== 'special'
          )
          const allChecked = standard.length > 0 && standard.every((item) => value[item.id] === true)
          return (
            <Checkbox
              disabled={readOnly}
              checked={allChecked}
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const permission = row.original.permissions.find(
            (item) => item.permissionType === 'read'
          )
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const permission = row.original.permissions.find(
            (item) => item.permissionType === 'create'
          )
          if (!permission) return <span className='text-slate-300'>-</span>
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const permission = row.original.permissions.find(
            (item) => item.permissionType === 'update'
          )
          if (!permission) return <span className='text-slate-300'>-</span>
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const permission = row.original.permissions.find(
            (item) => item.permissionType === 'delete'
          )
          if (!permission) return <span className='text-slate-300'>-</span>
    ...
    <div className='space-y-6'>
            ...
            return (
              <div key={category.id} className='mb-4 rounded-xl border border-slate-200'>
                <button
                  ...
                  onClick={() =>
                    setOpenCategories((previous) => ({ ...previous, [category.id]: !isOpen }))
                  }
                >
      ...
      {(() => {
        const specialsByCategory = categories
          .map((category) => {
            const modules = grouped.get(category.id) ?? []
            const specials = modules.flatMap((moduleItem) =>
              moduleItem.permissions
                .filter((permission) => permission.permissionType === 'special')
                .map((permission) => ({ ...permission, moduleName: moduleItem.moduleName }))
            )
