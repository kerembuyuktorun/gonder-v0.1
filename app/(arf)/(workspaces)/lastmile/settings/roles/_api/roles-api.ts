...
export async function fetchRoles(filters: FetchRolesFilters = {}): Promise<LastmileRole[]> {
  await latency()
...
export async function suspendRole(id: string): Promise<LastmileRole | undefined> {
  await latency(140)
  return setStoredRoleStatus(id, 'passive', 'UI Operator')
}

export async function activateRole(id: string): Promise<LastmileRole | undefined> {
  await latency(140)
...
export async function fetchModuleCategories(): Promise<ModuleCategory[]> {
  await latency(100)
  return [...MODULE_CATEGORIES].sort((a, b) => a.order - b.order)
}
