...
import { setCourierCostListStatus } from '../../../_lib/finance/courier-cost-service'
...
export async function PATCH(request: Request, context: Ctx) {
  ...
  if (body?.status !== 'active' && body?.status !== 'passive') {
    return NextResponse.json(
      { success: false, error: 'status active|passive olmalı' },
      { status: 400 }
    )
  }
  const item = await setCourierCostListStatus(auth.tenantId, id, body.status)
